...
def get_information(bv):
    resp = requests.get(
        f"https://www.bilibili.com/video/{bv}/?p=14&spm_id_from=pageDriver&vd_source=cd6ee6b033cd2da64359bad72619ca8a",
        headers=get_Header())
    # 提取视频oid
...
def start(bv, oid, pageID, count, csv_writer, is_second):
    ...
    for reply in comment['data']['replies']:
        ...
        # 写入CSV文件
        csv_writer.writerow(
            [count, sex, context, rereply, like])
...
def get_scv(bv= "BV19mqPB4Ec7"):
    ...
    # 创建CSV文件并写入表头
    with open('comment.csv', mode='w', newline='', encoding='utf-8-sig') as file:
        ...
        # 开始爬取
        while next_pageID != 0:
            bv, oid, next_pageID, count, csv_writer, is_second = start(bv, oid, next_pageID, count, csv_writer,
                                                                       is_second)
